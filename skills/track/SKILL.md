---
name: track
description: Aktualisiert den mitlaufenden Projekt-Tracker esc/docs/TRACKER.md von ESC aus dem aktuellen Zustand — Pipeline-Fortschritt (Mermaid), Artefakt-/Gate-Status, Story-Board und Decision-Log. Use when the user says "tracker aktualisieren", "fortschritt", "esc track", or wird am Ende jeder Phase automatisch von den anderen Skills aufgerufen.
allowed-tools: Read, Write, Bash
---

# esc:track — Projekt-Tracker aktualisieren

Ziel: `esc/docs/TRACKER.md` mit dem aktuellen Stand aus `esc/state.yaml` neu erzeugen. Schnell, idempotent.
...
2. **Fallback (manuell):** Meldet das Skript, dass es übersprungen hat (kein Python/nicht parsebar),
  oder ist es nicht vorhanden, dann erzeuge `esc/docs/TRACKER.md` selbst exakt nach der Vorlage in
  `shared/tracking.md`:
...
## Definition of Done
- [ ] `esc/docs/TRACKER.md` spiegelt `state.yaml` korrekt (Phase, Artefakte, Gates, Stories, Decisions).
